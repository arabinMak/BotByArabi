...
client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `salve @${num.split('@')[0]}\ bem vindo/a ao gp *${mdata.subject}* se apresenta e da uma mamadinha no arabe`
				...
				teks = `🤬ala mais um saiu e nao pagou a coca🤬 @${num.split('@')[0]} ate quando isso ?`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
		...
		try {
			...
			mess = {
				...
				only: {
					...
					ownerG: '❬🤬❭ ala querendo usar o comando do arabe.❬🤬❭ ',
					ownerB: '❬🤬❭  ala querendo usar o comando do arabe.❬🤬❭  ',
					admin: ' ❬ 🥴 ❭ ala achando que é adm kkk ❬ 🥴 ❭ ',
			...
			switch(command) {
                ...
                case 'hidetag':
					...
					if (!isOwner) return reply('sai dai mano kkk')
...
case 'lofi':
                 ...
                 case 'logoph':
					...
					var gbl1 = gh.split("|")[0];
					var gbl2 = gh.split("|")[1];
					if (args.length < 1) return reply('cade o txt safadao kkkk')
					reply(mess.wait)
                ...
                case 'logo3d':
                    ...
                    teks = `${body.slice(8)}`
                    if (teks.length > 10) return client.sendMessage(from, 'n e poema nao mano kkk', text, {quoted: mek})
			...
			case 'mapa': 
				...
				case 'clearall':
				case 'limpar':
					if (!isOwner) return reply(' af kkkk')
					anu = await client.chats.all()
					client.setMaxListeners(25)
					for (let _ of anu) {
						client.deleteChat(_.jid)
					}
					reply('esta limpo seu lindu  :)')
					break
			       case 'bloquear':
					if (!isGroup) return reply(mess.only.group)
					if (!isOwner) return reply(mess.only.ownerB)
				...
				case 'adicionar':
					...
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (args[0] === 'abrir') {
					    reply(`mais ja abriu o gp ? nem descansei`)
						client.groupSettingChange(from, GroupSettingChange.messageSend, false)
					} else if (args[0] === 'fechar') {
						reply(`grupo fechado agora deixa eu descansar`)
           ...
           case 'rebaixar':
			...
			case 'remover':
				...
				case 'boasvindas':
					...
					if (Number(args[0]) === 1) {
						if (isWelkom) return reply('agora vou falar bem vindo af ')
						welkom.push(from)
